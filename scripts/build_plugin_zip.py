...
VERSION_TEXT = (ROOT / "pymol_ligand_review" / "constants.py").read_text(encoding="utf-8")
VERSION = re.search(r'^PLUGIN_VERSION = "([^"]+)"', VERSION_TEXT, re.MULTILINE).group(1)
OUTPUT = ROOT / "dist" / f"PyMOL_Ligand_Review-{VERSION}.zip"


def files_to_package() -> list[Path]:
    files = sorted((ROOT / "pymol_ligand_review").glob("*.py"))
